...
def upgrade_to_luxury(day: dict, destination: str) -> dict:
    ...
    if "hotel" in upgraded.get("morning", "").lower() or "check-in" in upgraded.get("morning", "").lower():
        upgraded["morning"] = upgraded["morning"].replace("hotel", "luxury 5-star hotel").replace("check-in", "VIP check-in at luxury resort")
    if "walk" in upgraded.get("morning", "").lower() or "explore" in upgraded.get("morning", "").lower():
        upgraded["morning"] = f"Private guided tour of {destination}'s exclusive morning spots with luxury transport"
    if "market" in upgraded.get("morning", "").lower():
        upgraded["morning"] = f"Private shopping experience at {destination}'s high-end boutiques with personal shopper"
    
    if "museum" in upgraded.get("afternoon", "").lower() or "tour" in upgraded.get("afternoon", "").lower():
        upgraded["afternoon"] = f"Private VIP museum tour with expert guide, skip-the-line access"
    elif "beach" in upgraded.get("afternoon", "").lower() or "relax" in upgraded.get("afternoon", "").lower():
        upgraded["afternoon"] = "Private beach cabana with butler service and premium amenities"
    elif "sightsee" in upgraded.get("afternoon", "").lower() or "visit" in upgraded.get("afternoon", "").lower():
        upgraded["afternoon"] = f"Luxury private car tour of {destination}'s landmarks with champagne service"
    else:
        upgraded["afternoon"] = f"Exclusive high-end experience: {upgraded.get('afternoon', 'Leisure time')} with premium service"
    
    if "dinner" in upgraded.get("evening", "").lower() or "restaurant" in upgraded.get("evening", "").lower():
        upgraded["evening"] = f"Michelin-starred dinner experience at {destination}'s finest restaurant with wine pairing"
    elif "show" in upgraded.get("evening", "").lower() or "entertainment" in upgraded.get("evening", "").lower():
        upgraded["evening"] = f"VIP box seats at premium show with backstage access and champagne"
    else:
        upgraded["evening"] = f"Sunset cocktail reception at luxury rooftop bar followed by gourmet dining"
    
    if "local" in upgraded.get("food", "").lower() or "street" in upgraded.get("food", "").lower():
        upgraded["food"] = f"Fine dining: {upgraded['food'].replace('street food', 'gourmet cuisine').replace('local', 'premium local')}"
    else:
        upgraded["food"] = f"Gourmet dining experience: {upgraded.get('food', ' chef tasting menu')}"
    
    if upgraded.get("notes"):
        upgraded["notes"] = f"All bookings include VIP service and premium transfers. {upgraded['notes']}"
    else:
        upgraded["notes"] = "Includes private transfers, priority access, and concierge service"
...
def downgrade_to_budget(day: dict, destination: str) -> dict:
    ...
    if "luxury" in upgraded.get("morning", "").lower() or "private" in upgraded.get("morning", "").lower():
        upgraded["morning"] = upgraded["morning"].replace("luxury", "comfortable").replace("private", "self-guided")
    if "tour" in upgraded.get("morning", "").lower():
        upgraded["morning"] = f"Self-guided walking tour of {destination} using free map app"
    
    if "vip" in upgraded.get("afternoon", "").lower() or "private" in upgraded.get("afternoon", "").lower():
        upgraded["afternoon"] = upgraded["afternoon"].replace("VIP", "Standard").replace("private", "group")
    if "car" in upgraded.get("afternoon", "").lower() or "transport" in upgraded.get("afternoon", "").lower():
        upgraded["afternoon"] = f"Public transport exploration of {destination} (buy day pass)"
    
    if "michelin" in upgraded.get("evening", "").lower() or "fine dining" in upgraded.get("evening", "").lower():
        upgraded["evening"] = "Casual dinner at highly-rated local eatery or food market"
    
    upgraded["food"] = f"Budget-friendly: {upgraded.get('food', 'local street food and markets')}"
    
    upgraded["notes"] = "Budget travel tips: Use public transport, book attractions online for discounts"
...
def make_day_relaxing(day: dict, day_num: int, destination: str) -> dict:
    ...
    relaxed["morning"] = f"Late wake-up at {destination} resort, leisurely breakfast with scenic views, optional yoga session"
    relaxed["afternoon"] = f"Spa treatment and wellness activities OR peaceful {destination} gardens/park visit with picnic"
    relaxed["evening"] = "Sunset viewing at quiet spot, early dinner at peaceful restaurant, stargazing or reading"
    relaxed["food"] = "Healthy, organic meals at wellness café or resort restaurant"
    relaxed["notes"] = "No rush today - move at your own pace. Spa appointments recommended. Perfect for recharging."
...
def create_smart_modification(user_message: str, current_itinerary: dict, preferences: dict) -> Dict:
    """
    ...
    if "days" not in modified_itinerary or not isinstance(modified_itinerary["days"], list):
    ...
    if any(phrase in message_lower for phrase in ["add day", "extend trip", "one more day", "extra day", "additional day"]):
        should_add_day = True
    ...
    if should_add_day and days:
        ...
        if add_day_relaxing:
            ...
            changes_made.append(f"Added relaxing Day {new_day_num} with spa and wellness activities")
        else:
            # Generic new day
            new_day = {
                "day": new_day_num,
        ...
        days.append(new_day)
        modified_itinerary["overview"] = modified_itinerary.get("overview", "") + f" Extended to {new_day_num} days."
    
    # 2. HANDLE MODIFY EXISTING DAYS (e.g., "update day 2", "make day 2 relaxing")
    specific_day_match = re.search(r'(?:update|modify|change)\s+day\s*(\d+)', message_lower)
    ...
    if specific_day_match:
        ...
        if 1 <= day_to_modify <= len(days):
            if make_relaxing:
                days[day_to_modify - 1] = make_day_relaxing(days[day_to_modify - 1], day_to_modify, destination)
                changes_made.append(f"Converted Day {day_to_modify} to a relaxing schedule")
            elif "luxury" in message_lower or "high" in message_lower:
                days[day_to_modify - 1] = upgrade_to_luxury(days[day_to_modify - 1], destination)
                changes_made.append(f"Upgraded Day {day_to_modify} to luxury experiences")
    ...
    if "accordingly" in message_lower or "update" in message_lower:
        ...
        if add_day_relaxing and len(days) >= 2:
            ...
            if "hike" in days[prev_day_idx].get("morning", "").lower() or "busy" in days[prev_day_idx].get("notes", "").lower():
                days[prev_day_idx]["notes"] = "Adjusted for better pacing before relaxation day. " + days[prev_day_idx].get("notes", "")
                changes_made.append(f"Adjusted Day {prev_day_idx + 1} for better trip flow")
    
    # 4. HANDLE BUDGET UPDATES
    if "budget" in message_lower or "luxury" in message_lower:
        if any(word in message_lower for word in ["high", "luxury", "expensive", "upgrade"]):
            modified_itinerary["days"] = [upgrade_to_luxury(day, destination) for day in days]
            ...
            modified_itinerary["days"] = [downgrade_to_budget(day, destination) for day in days]
            changes_made.append("Adjusted itinerary for budget-conscious travel")
    
    # Build response message
    if changes_made:
        response_msg = "I've made the following changes to your itinerary: " + "; ".join(changes_made) + "."
    else:
        response_msg = f"I've processed your request: '{user_message}'. Your itinerary has been adjusted where possible."
    
    return {
        "response_message": response_msg + " (Note: AI temporarily unavailable, using smart fallback)",
...
"""
            ...
            if (
                ...
                if isinstance(updated_itinerary.get("days"), list):
                    for i, day in enumerate(updated_itinerary["days"]):
                        ...
                        for field in ["title", "morning", "afternoon", "evening", "food", "notes"]:
    ...
    fallback = create_smart_modification(
        user_message,
        current_itinerary,
        preferences
    )
...
def process_chat_and_update(
    ...
    print(
        f"Updated itinerary + preferences for itinerary {itinerary_id}"
    )
